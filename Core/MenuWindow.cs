...
namespace Exile
{
    public class MenuWindow : IDisposable
    {
        ...
        public MenuWindow(Core core, SettingsContainer settingsContainer, Dictionary<string, FontContainer> fonts) {
            ...
            CoreSettings = () =>
            {
                foreach (var drawer in CoreSettingsDrawers) drawer.Draw();
            };
            _index = -1;
            Selected = CoreSettings;
        ...
        public List<ISettingsHolder> CoreSettingsDrawers { get; private set; }
        ...
        private int PluginNameWidth = 200;
        private Action CoreSettings = () => { };
        ...
        private Action OnWindowChange;
        private DebugInformation AllPlugins = null;
        private Windows openWindow;
        static Stopwatch swStartedProgram = Stopwatch.StartNew();
...
/*            for (var index = 0; index < ((Windows[]) WindowsName).Length; index++)
            ...
            switch (OpenWindow)
            {
                case Windows.MainDebugs:
                {
                    ...
                    ImGui.NextColumn();
                    foreach (var deb in MainDebugs) DrawInfoForDebugInformation(deb, Core.DebugInformations[0], MainDebugs.Count);
                ...
                case Windows.NotMainDebugs:
                {
                    ...
                    ImGui.Text($"Data for {DebugInformation.SizeArray / _CoreSettings.TargetFps} sec.");
                    ImGui.NextColumn();
                    foreach (var deb in NotMainDebugs) DrawInfoForNotMainDebugInformation(deb);
                    ImGui.Columns(1, "", false);
                    break;
                }
                ...
                case Windows.Plugins:
                {
                    ...
                    foreach (var deb in PluginsDebug) DrawInfoForDebugInformation(deb, AllPlugins, PluginsDebug.Count);
