...
namespace Shared
{
    public class PluginManager
    {
        ...
        private List<Assembly> GetCompiledAssemblies(DirectoryInfo[] compiledPlugins, bool parallel = false) {
        ...
        private Dictionary<string, Stopwatch> PluginLoadTime = new Dictionary<string, Stopwatch>();

        private object lockerLoadAsm = new object();
        ...
        private Assembly LoadAssembly(DirectoryInfo dir) {
            try
            {
                ...
                var dll = directoryInfo.GetFiles($"{directoryInfo.Name}*.dll", SearchOption.TopDirectoryOnly).FirstOrDefault();
                if (dll == null)
                {
        ...
        private void TryLoadAssemble(Assembly asm, bool fromSource) {
            try
            {
                ...
                var fullPath = Path.Combine(fromSource ? Directories[SourcePluginsDirectory] : Directories[CompiledPluginsDirectory], dir);
            ...
            {
               LogError($"Error when load plugin: {e}");
            }
        }
        ...
        public void CloseAllPlugin() {
            foreach (var plugin in Plugins) plugin.Close();
        }
        ...
        private void EntityListWrapperOnEntityAdded(Entity entity) {
            if (_gameController.Settings.CoreSettings.AddedMultiThread && _multiThreadManager.ThreadsCount > 0)
            {
                ...
                Plugins.WhereF(x=>x.IsEnable).Batch(_multiThreadManager.ThreadsCount)
                    .ForEach(wrappers => listJob.Add( _multiThreadManager.AddJob(() => wrappers.ForEach(x => x.EntityAdded(entity)), "Entity added")));
        ...
        private void LogError(string msg) => DebugWindow.LogError(msg);
    }
}