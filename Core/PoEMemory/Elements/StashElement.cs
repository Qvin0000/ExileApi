...
namespace PoEMemory
{
    public class StashElement : Element
    {
        ...
        private Inventory GetVisibleStash() => GetStashInventoryByIndex(IndexVisibleStash);

        public IList<string> AllStashNames => GetAllStashNames();

        private List<string> GetAllStashNames() {
            var ret = new List<string>();
            for (var i = 0; i < TotalStashes; i++) ret.Add(GetStashName(i));
            return ret;
        }
        ...
        private IList<Inventory> GetAllInventories() {
            ...
            for (var i = 0; i < TotalStashes; i++) result.Add(GetStashInventoryByIndex(i));
