...
namespace PoEMemory
{
    ...
    {
        private HashSet<long> hashSet = new HashSet<long>(256);
        private Queue<long> queue = new Queue<long>(256);
        private object locker = new object();
        public int EntitiesProcessed { get; private set; }
         private List<long> hashAddresses = new List<long>(1000);

        HashSet<long> StoreIds = new HashSet<long>(256);
        WaitTime collectEntities = new WaitTime(1);
        ...
        public IEnumerator CollectEntities(EntityCollectSettingsContainer container) {
                ...
                {
                    foreach (var addrEntity in hashAddresses) StoreIds.Add(ParseEntity(addrEntity, container.EntityCache, container.EntitiesVersion, container.Simple,parseServerEntities));
                }
