...
namespace Exile.PoEMemory.MemoryObjects
{
    public class Entity : RemoteMemoryObject
    {
        ...
        public override string ToString() =>
            $"#{Id} <{Type}> ({Rarity}) {Metadata}: ({Address:X})";
        ...
        public MonsterRarity Rarity =>(MonsterRarity) (_rarity ??= GetComponent<ObjectMagicProperties>()?.Rarity ?? MonsterRarity.White);
        ...
        public float Distance(Entity entity) => GridPos.Distance(entity.GridPos);
        ...
        ValidCache<List<Buff>> buffCache;
        ...
        private bool CheckRarity() => Rarity >= MonsterRarity.White && Rarity <= MonsterRarity.Unique;
        ...
        public void UpdatePointer(long newAddress) => Address = newAddress;


        private int pathReadErrorTimes = 0;
                ...
                while (!AllStatsParsed && tries < 3)
                {
                    ...
                    while (node.Next != lastComponent)
                    {
                        if (startTime > 30)
                        {
                            ...
                            foreach (var l in result) DebugWindow.LogError($"{l.Key} : {l.Value:X}");
                            return result;
                        }
        ...
        public bool HasComponent<T>() where T : Component, new() =>
            CacheComp != null && CacheComp.TryGetValue(typeof(T).Name, out var address) && address != 0;
        ...
        private EntityType ParseType() {
            ...
            if (HasComponent<MinimapIcon>())
            {
                ...
                if (Path.Equals("Metadata/MiscellaneousObjects/Delve/DelveCraftingBench", StringComparison.Ordinal))return EntityType.DelveCraftingBench;
